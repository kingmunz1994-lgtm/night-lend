/**
 * deploy.ts — Deploy NightLend to Midnight Preprod
 *
 * Run:
...
const ZK_CONFIG_PATH = path.resolve(import.meta.dirname, '..', 'contracts', 'managed', 'night-lend');
...
async function main() {
  ...
  console.log('\n🚀 Deploying NightLend to Midnight Preprod...\n');
  ...
  const compiled = CompiledContract
    .make('night-lend', ContractModule.Contract)
  ...
  const providers = {
    ...
    privateStateProvider: levelPrivateStateProvider({ privateStateStoreName: 'night-lend-deploy', walletProvider: ctx.wallet as any }),
  ...
  const deployed = await deployContract(providers, { compiledContract: compiled, privateStateId: 'nightLendState', initialPrivateState: {} });
